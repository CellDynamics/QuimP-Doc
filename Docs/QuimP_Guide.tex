...
\begin{enumerate}
	\item \href{https://pilip.lnx.warwick.ac.uk/site/changes-report.html#a19.04.01}{v19.04.01}
	\begin{itemize}
		\item Added Omero import/export option (\autoref{sec:Omero}).
	\end{itemize}
	\item \href{https://pilip.lnx.warwick.ac.uk/site/changes-report.html#a19.03.01}{v19.03.01}
	\begin{itemize}
		\item Reworked Protrusion Tracking module (\autoref{sec:ProtTracking}).
		\item Python examples of processing \textit{QCONF} at \href{https://github.com/CellDynamics/QuimP-Python}{GitHub account}\footnote{https://github.com/CellDynamics/QuimP-Python}
	\end{itemize}
...
Simple example of processing QuimP datafiles in Python is available at our  \href{https://github.com/CellDynamics/QuimP-Python/blob/master/qconf_examples.ipynb}{GitHub account}\footnote{https://github.com/CellDynamics/QuimP-Python/blob/master/qconf\_examples.ipynb}
...
\begin{enumerate}
	...
	\item Omero client - available from $\textit{Tools}\rightarrow\textit{Omero client}$ menu (\autoref{sec:Omero})
\end{enumerate}
...
\subsection{Omero integration}
\label{sec:Omero}

QuimP allows for simple integration with Omero database. Currently supported is uploading and downloading \textit{QCONF} experiment files to and from Omero. In both cases QCONF file and related image are loaded or saved to disk. For more detail consult short help available in OmeroClient module ($\textit{Tools}\rightarrow\textit{Omero client}$). 
...
Protrusion Analysis module allows investigating protrusion formation and dynamics. The module requires \textit{QCONF} file that contains full description of QuimP analysis. Particularly, the BOA, ECMM an Q Analysis modules should be run on \textit{QCONF} before using Protrusion Analysis.

Arbitrary points of interest can be selected directly on the cell contour or imported from \textit{ROI Manager}. 
...
\begin{enumerate}
	\item \textbf{Visual tracking} - allow to define points of interests either directly on the cell contour or import them from ROI manager. See note below about coordinate systems and importing from ROI scenarios. Tracking also produces \textit{tracks.csv} files with tracks.
	\item \textbf{Maps} - display \textit{Motility}, \textit{Convexity} or \textit{Fluorescence} maps for selected cell.
	\item \textbf{Tables and plots} - produce IJ Result tables with statistics for selected cell (\textit{Geom} button) or fluorescent channel (\textit{Fluo} button). These are the same data as included in the\textit{ .csv} file generated and updated by QuimP pipeline (BOA, ECMM, ANA, Q-Analysis).
	\item \textbf{Polar plots} - generate and save in current folder polar plots. Polar plot represents local velocity of the cell membrane averaged over time for each node (owing ECMM algorithm, the cell contour can be interpolated to any real number of nodes that can be tracked forward and backward in time). Point on the contour that is closest to location of chemotactic gradient source is considered as the first one at polar coordinate $\theta=0^{\circ}$.  Chemotactic gradient source can be selected by mouse (\textit{Click} button), from ROI (\textit{ROI} button - first point of the ROI is considered as source) or from predefined settings (screen corners).  
\end{enumerate}

\subsection{Coordinates and conversions}
\label{sec:Coordinates}

The contours of the cell consist of vertices, internally each vertex \textit{v} for cell \textit{c} is represented by two coordinates: position along outline and time (frame), $v=<p;t>$. The same coordinate system is used for maps generated by Q-Analysis (\textbf{Maps} section of the UI). The vertical axis represents 
time, with frame 1 at the top.  The horizontal axis represents positions along the cell outline, the length of which has been normalised to 1 at every frame (see \autoref{maps}). Resolution of horizontal axis can be configured in Q-Analysis module (default value is 400). Resolution of vertical axis equals number of frames in the sequence. These both values are stored in the \textit{QCONF} file.

If points selected on the cell contour in the main view of the plugin are exported to ROI manager, they are converted to screen coordinates using configured resolution for positions along the cell outline and number of frames in the sequence (e.g. for default resolution of 400 and 100 frames, point selected at frame 50 in the middle of the contour will have coordinates $x = 200$, $y = 50$). Additionally cell number is encoded in the ROI name as suffix \textit{cell\_N\_X}, where \textit{N} is the cell number and \textit{X} stands for point number.

Reverse operation - importing points from ROI expects the same naming convention unless the user will press and hold \textit{CTRL} key to import all points to the outline of the cell 0. ROI coordinates also should follow guidances given above, they will be converted to <position;frame> on the outline. Position is selected as the closest vertex on the outline to the input point. Not that ROI can be also shapes, which allows to select more than one point at once.

\subsection{Generate tracks}
\label{sec:GenerateTracks}
...
\begin{enumerate}
	\item Hold \textit{CTRL} and click points of interest on the outlines. You can select points on different cells and different frames. Use \textit{Remove all} button to delete all selected points.
	\item Click Track to see forward (green) and backward (yellow) tracks. Additional options affect how the output information is presented to the user. For details see tooltips.
	\item Investigate \textit{tracks\_N.csv} in the root folder. It contains indexes that index maps (\autoref{maps}) and can be used for extracting e.g. values of motility or fluorescence, e.g. one can use Format Converter to generate \textit{csv} representation of maps (see \autoref{sec:ConversionBetweenFormats} and \autoref{sec:walk_macro}).
\end{enumerate}

\subsubsection{Importing from ROI}
\label{sec:ImportingFromROI}
...
\begin{enumerate}
	\item Load \textit{QCONF} file ti the plugin.
	\item In \textbf{Maps} section select cell and click \textit{Mot} button to open motility map.
	\item From Fiji open \texttt{Process$\rightarrow$Find Maxima} plugin and try to identify points in the motility map of highest local velocity (you might need to select options: \textit{Preview}, \textit{Light background} and use threshold around 50). Click Ok if you are happy with results.
	\item Add points to the ROI manager (Ctrl+T).
	\item Return to the Protrusion Analysis module and click \textit{<-ROI} to import points to program. Note that by default points are assigned to cell 0, if it is another cell take care about proer naming of the ROI in ROI manager.
	\item Click Track to see forward (green) and backward (yellow) tracks. Additional options affect how the output information is presented to the user. For details see tooltips.
	\item Investigate \textit{tracks\_N.csv} in the root folder. It contains indexes that index maps (\autoref{maps}) and can be used for extracting e.g. values of motility or fluorescence, e.g. one can use Format Converter to generate \textit{csv} representation of maps (see \autoref{sec:ConversionBetweenFormats} and \autoref{sec:walk_macro}).
\end{enumerate}
...
The latest QuimP introduces a new file format called QCONF\footnote{above QuimP11}. The \textit{QCONF} is based on human-readable JSON format and it is designed to hold all results evaluated by QuimP on every stage of data processing, together with configuration parameters, algorithm settings, etc. (check \autoref{sec:ProcessingDataLanguages} for details on QCONF structure).
...
\subsubsection{Processing data files in other languages}
\label{sec:ProcessingDataLanguages}

Simple example of processing QuimP data files in Python is available at our  \href{https://github.com/CellDynamics/QuimP-Python/blob/master/qconf_examples.ipynb}{GitHub account}\footnote{https://github.com/CellDynamics/QuimP-Python/blob/master/qconf\_examples.ipynb}.Notebook contains also scheme of \textit{QCONF} structure generated by tool available in the same repository. 

Similar approach can be used in any language supporting reading JSON files. One can also convert \textit{QCONF} to old file format, based mostly on widely understood \textit{csv} files, and follow e.g. this example in \href{https://pilip.lnx.warwick.ac.uk/docs/Walkthrough.html}{Matlab}\footnote{https://pilip.lnx.warwick.ac.uk/docs/Walkthrough.html}  
