...
\begin{enumerate}
	...
	\begin{itemize}
		\item Enhanced macro support - see \ref{sec:walk_macro}
	...
	\begin{itemize}
		...
		\item Added option in BOA for expanding snakes from cell inside - see \ref{sec:BOAPlugin}
		\item Significant changes in Random Walk module, e.g. multi object segmentation - see \ref{sec:RWSeg} 
	\end{itemize}
...
You can view them in ImageJ by opening them via $[File -> Import -> Text Image...]$. Note that latest versions of QuimP store these files inside \textit{QCONF} configuration file. One can retrieve them by using Format Converter (see Chapter \ref{sec:DataFilesExplained}).
...
In this example you will perform analysis similar to Chapter \ref{walkthrough} but with help of ImageJ macro. Note that this approach requires already segmented image provided as binary or grayscale mask (we use \textit{Segmentation.tif} from walkthrough files). More about converting masks to QuimP format is given in Chapter \ref{sec:Binary_seg}. Full macro is available in walkthrough data archive.
...
\begin{enumerate}
	...
	% TODO Add new converter and link to sec:Binary_seg
\end{enumerate}
...
The RW plugin can be called outside of QuimP from the QuimP Bar, from IJ macro or directly from API. The main interface is presented in Figure \ref{fig:rwdialog}. The window is divided into eight sections:
\begin{enumerate}
	\item Image section
	\item Seeds section
...
\begin{itemize}
	...
	\item ROIs - initial seeds can be provided as ROIs. This mode also supports multi-object segmentation. There is simple tool available for collecting ROIs, described in section \ref{sec:CollectingROIs}.   
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \textit{Norm dist} - approximate number of nodes used for normales equalization. The algorithm looks for node with minimal curvature within this range and then set normales for all nodes in processed window to be same as in this node. Refer to Figure \ref{fig:shrinkcomp} for examples of \textit{Curv dist} and \textit{Norm dist} parameters.  
		\end{itemize}
...
The ROI collector tool (Fig. \ref{fig:RoiTool}) manages proper naming of ROIs in the ROI Manager. After launching, the ROI Manager will be opened and its content erased. 
...
Note that macro strings are outputed in Macro Recorder only if plugin is run from Menu. \textbf{It does not work if plugin has been started from QuimP Toolbar.}
