...
The Format converter module allows to save selected parameters evaluated during QuimP workflow as \textit{csv} files. They are not processed in any way (e.g. scaled to different units) and mostly coincide with content of \textit{snQP} or \textit{stQP.csv} files. This tool works only wit \textit{QCONF} files that should be first loaded by \textit{Load QCONF} button. Conversion process is initiated by \textit{Generate} button. Output files are saved in the same folder as input \textit{QCONF}.    
