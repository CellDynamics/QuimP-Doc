...
\noindent\fcolorbox{red}{yellow}{
	\parbox{\textwidth} {
		Matlab procedures works with old data format (\textit{*.paQP}, \textit{*.snQP}, etc.). To use them with latest \textit{QCONF} data file, it has to be first converted to old format. It is also possible to load \textit{QCONF} to Matlab using JSon readers.  
	}
}
